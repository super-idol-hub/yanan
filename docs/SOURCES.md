...
首发“黑白日常”服装是本项目提出并经用户确认的无品牌服装方案，不宣称对应某次真实活动。身份锚点通过内置 ImageGen 生成；用户已明确回复“确认这个形象和服装，继续制作动作”。

2026-09-24 用户提供四张实拍参考，指定第一张为面部基准，并明确回复“三套皮肤都按这张脸更新”，随后要求脸部更写实。后三张仅作为“绯红舞台”的黑红外套、白色内搭、层叠阔腿裤、耳麦和饰品参考。本次三套动作统一使用该面部参考与对应生成锚点；原始上传照片不进入仓库、EXE、ZIP 或 CI。生成方式与完整提示词见 `PHOTO_IDENTITY_REVISION.json`。
